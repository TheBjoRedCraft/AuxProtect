...
public class LookupCommand<S, P extends IAuxProtect, SA extends SenderAdapter<S, P>> extends
  ...
  public static List<String> onTabCompleteStatic(IAuxProtect plugin, SenderAdapter<?, ?> sender,
    ...
    if (currentArg.startsWith("user:") || currentArg.startsWith("u:") || currentArg.startsWith(
      ...
      possible.addAll(
          APCommand.allPlayers(plugin, true).stream().map(name -> user + name).toList());
  ...
  public void onCommand(SA sender, String label, String[] args) throws CommandException {
    ...
    try {
      ...
      if (plugin.getAPConfig().isDemoMode() && !sender.isConsole()) {
        rs.removeIf(entry -> {
          try {
            return entry.getUserUUID().startsWith("$") && !entry.getUserUUID()
  ...
  protected void handleResults(Parameters params_, List<DbEntry> rs, SA sender)
    ...
    if (params_.hasFlag(Flag.COUNT)) {
      ...
      sender.sendMessage(PlayTimeSolver.solvePlaytime(plugin, rs,
          params_.getAfter(),
