...
public class Language {
  ...
  public static void load(IAuxProtect plugin, Supplier<YamlConfig> langSupplier,
      Supplier<YamlConfig> defaultLangSupplier) throws IOException {
  ...
  public static void reload() throws IOException {
    ...
    if (resourceVersion > 0 && resourceVersion > fileVersion) {
      AuxProtectAPI.info(
          "Resetting language file from v" + fileVersion + " to v" + resourceVersion);
      lang.delete();
      lang.load();
    }
  ...
  public enum L {
    ...
    COMMAND__TIME__CLICK_TO_COPY,
    COMMAND__TIME__EPOCH_TIME,
    COMMAND__TIME__FORMATTED,
    COMMAND__TIME__RELATIVE,
    COMMAND__TIME__RELATIVE_EXPANDED,
        ...
        for (String line : format) {
          if (!line.matches("[a-zA-Z0-9_-]+")) {
            throw new IllegalArgumentException(
                "Non alphanumeric translation key: " + line + " in " + name);
          }
        }
      }
    }
      ...
      try {
        ...
        if (message == null && defaultLang != null) {
          ...
          if (message != null) {// && !Objects.equals(getLocale(), "en-us")) {
            plugin.warning(
                "Failed to find lang `" + name + "` in loaded lang file, used default en-us lang.");
          }
        }
        if (message == null) {
          throw new IllegalArgumentException("Message not found: " + name);
        }
        ...
        if (format != null) {
          if (this.format.length != format.length) {
            throw new IllegalArgumentException(
                "Mismatched format lengths " + this.format.length + "!=" + format.length);
          }
        ...
        if (plugin != null) {
          plugin.warning("Failed to find lang `" + name + "`");
        }
