...
public class MigrationManager {
  ...
  MigrationManager(SQLManager sql, Connection connection, IAuxProtect plugin) {
    ...
    migrationActions.put(16, new MigrationAction(true, () -> {
      ...
      for (Table table : Table.values()) {
        if (!table.hasAPEntries() && table != Table.AUXPROTECT_INVDIFF) {
          continue;
        }
        if (!table.exists(plugin)) {
          continue;
        }
        sql.execute(connection, "UPDATE " + table + " set time=time*? WHERE time<?",
            Snowflake.COUNTER_FACTOR, 1735689600000L * Snowflake.COUNTER_FACTOR);
      }
    }));
    ...
    migrationActions.put(17, new MigrationAction(true, () -> {
      ...
      sql.execute(connection, sql.getDropIndexStatement("idx_" + Table.AUXPROTECT_UIDS + "_hash",
          Table.AUXPROTECT_UIDS.toString(), true));
      sql.execute(connection,
          "ALTER TABLE " + Table.AUXPROTECT_UIDS + " RENAME TO " + Table.AUXPROTECT_UIDS
              + "_aptemp");
    }, () -> {
      // UID migrations
      sql.execute(connection, sql.getInsertOrIgnore() + " INTO " + Table.AUXPROTECT_UIDS
          + " (id,value) SELECT uid,uuid FROM " + Table.AUXPROTECT_UIDS + "_aptemp");
      sql.execute(connection, "DROP TABLE " + Table.AUXPROTECT_UIDS + "_aptemp");

      // Chat migrations
      if (plugin.getSqlManager().isMySQL()) {
        sql.execute(connection, "ALTER TABLE " + Table.AUXPROTECT_CHAT
            + " CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci");
      }
      if (plugin.getSqlManager().isMySQL()) {
        sql.execute(connection, "ALTER TABLE " + Table.AUXPROTECT_CHAT
            + " ADD COLUMN data LONGTEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci");
      } else {
        sql.execute(connection,
            "ALTER TABLE " + Table.AUXPROTECT_CHAT + " ADD COLUMN data LONGTEXT");
      }
      sql.execute(connection,
          "ALTER TABLE " + Table.AUXPROTECT_CHAT + " ADD COLUMN target_id INTEGER");
    ...
    migrationActions.put(18, new MigrationAction(true, () -> {
      ...
      for (Table table : Table.values()) {
        ...
        if (limit == 0) {
          throw new IllegalStateException("Duplicate snowflakes detected in " + table
              + ". Failed to fix them after 1000 attempts.");
        }

        plugin.info("Removing old/temp time index on " + table);
        sql.execute(connection,
            sql.getDropIndexStatement("idx_" + table + "_time", table.toString(), true));
        sql.execute(connection,
            sql.getDropIndexStatement("idx_" + table + "_action_time", table.toString(), true));
      }
    }, () -> {
      for (Table table : Table.values()) {
        if (!table.hasBlob()) {
        ...
        if (sql.columnExists(connection, table.toString(), "ablob")) {
          continue;
        }
      ...
      sql.execute(connection,
          "ALTER TABLE " + Table.AUXPROTECT_LONGTERM + " ADD COLUMN target_id INT");

      plugin.info("Migrating longterm to UIDs. Creating temp index.");
      sql.execute(connection,
          "CREATE INDEX idx_auxprotect_longterm_target ON " + Table.AUXPROTECT_LONGTERM
              + " (target)");
      plugin.info("Migrating uids");
      while (true) {
        var toUpdate = sql.query(connection,
            "SELECT DISTINCT target FROM " + Table.AUXPROTECT_LONGTERM
                + " WHERE target IS NOT NULL LIMIT 10000", rs -> {
        ...
        Map<String, Integer> toUpdateMap = new HashMap<>();
        for (String value : toUpdate) {
          int id = sql.getUidManager().getIDOrInsert(connection, value);
          toUpdateMap.put(value, id);
        }

        sql.executeBatch(
            connection,
            "UPDATE " + Table.AUXPROTECT_LONGTERM + " SET target_id=?, target=null WHERE target=?",
            toUpdateMap.entrySet(),
            e -> List.of(e.getValue(), e.getKey())
        );
      }
      ...
      sql.execute(connection, sql.getDropIndexStatement("idx_auxprotect_longterm_target",
          Table.AUXPROTECT_LONGTERM.toString(), false));
    }));

    migrationActions.put(19, new MigrationAction(true, () -> {
      plugin.info("Fixing duplicate UID values");
      sql.query(connection, "SELECT id,value,count(1) AS ct FROM " + Table.AUXPROTECT_UIDS
          + " GROUP BY lower(value) HAVING ct>1", rs -> {
        while (rs.next()) {
          int id = rs.getInt("id");
          String value = rs.getString("value");
          int count = rs.getInt("ct");
          sql.execute(connection, "UPDATE " + Table.AUXPROTECT_UIDS + " SET value=? WHERE id=?",
              value + "_" + count, id);
        }
      });
    }, () -> {
    ...
    if (max != TARGET_DB_VERSION) {
      throw new IllegalArgumentException(
          "Improperly defined migration actions. DBVERSION=" + TARGET_DB_VERSION
              + " with max action=" + max);
    }
  }
  ...
  public String getProgressString() {
    ...
    if (migratingToVersion <= 0) {
      return null;
    }
    int progressPercentage = (int) Math.floor((double) getComplete() / getTotal() * 100);
    return String.format("Migration to v%d %d%% complete. (%d/%d). DO NOT INTERRUPT",
        migratingToVersion, progressPercentage, getComplete(), getTotal());
  }

  private void setVersion(int version) throws SQLException {
    sql.execute(connection,
        "INSERT INTO " + Table.AUXPROTECT_VERSION + " (time,version) VALUES (?,?)",
  ...
  void preTables() throws SQLException {
    ...
    sql.execute(connection, "CREATE TABLE IF NOT EXISTS " + Table.AUXPROTECT_VERSION
        + " (time BIGINT, version INTEGER)");

    sql.query(connection,
        "SELECT * FROM " + Table.AUXPROTECT_VERSION + " ORDER BY time DESC LIMIT 1", rs -> {
          if (rs.next()) {
            version = rs.getInt("version");
          }
        });
    sql.query(connection,
        "SELECT * FROM " + Table.AUXPROTECT_VERSION + " ORDER BY time ASC LIMIT 1", rs -> {
          if (rs.next()) {
    ...
    if (currentVersion < 6) {
      plugin.warning(
          "This database version is no longer supported. Please download AuxProtect 1.2.7 and run it first to upgrade your database, wait for migration to complete, then run this version again. https://www.spigotmc.org/resources/auxprotect.99147/download?version=509785");
      throw new SQLException();
    }

    if (currentVersion < 15) {
      plugin.warning(
          "This database version is no longer supported. Please download AuxProtect 1.3.3 and run it first to upgrade your database, wait for migration to complete, then run this version again. https://www.spigotmc.org/resources/auxprotect.99147/download?version=575276");
      throw new SQLException();
    }

    if (sql.getVersion() < TARGET_DB_VERSION) {
      plugin.info("Outdated DB Version: " + sql.getVersion() + ". Migrating to version: "
          + TARGET_DB_VERSION + "...");
      ...
      for (int i = sql.getVersion() + 1; i <= TARGET_DB_VERSION; i++) {
        ...
        if (action == null) {
          plugin.warning(
              "This database version is no longer supported. Please download an older AuxProtect version and run it first to upgrade your database, wait for migration to complete, then run this version again. https://www.spigotmc.org/resources/auxprotect.99147");
          throw new SQLException();
        }
      }
    }
  ...
  private void migrateDataNormalization() throws SQLException {
    if (migrationTaskSet.contains(connection, OptionalMigrationTask.DATA_NORMALIZATION)) {
      plugin.info(
          "Data already normalized, skipping. You can unset \"MigrateDataNormalization\" in config.yml to remove this message.");
      return;
    }
    ...
    // Migrate IPs to UID
    {
      ...
      sql.execute(connection, "CREATE INDEX idx_auxprotect_main_data ON " + Table.AUXPROTECT_MAIN
          + " (action_id,data)");

      plugin.info("Migrating IPs");
      while (true) {
        var toUpdate = sql.query(connection, "SELECT DISTINCT data FROM " + Table.AUXPROTECT_MAIN
            + " WHERE action_id=5 AND data IS NOT NULL LIMIT 10000", rs -> {
          Set<String> newIDs = new HashSet<>();
          while (rs.next()) {
        ...
        sql.executeBatch(
            connection,
            "UPDATE " + Table.AUXPROTECT_MAIN
                + " SET target_id=?, data=null WHERE action_id=5 AND (data=? OR data=?)",
            toUpdateMap.entrySet(),
            e -> List.of(e.getValue(), e.getKey(), "IP: " + e.getKey())
        );
      }

      plugin.info("Removing temp index");
      sql.execute(connection,
          sql.getDropIndexStatement("idx_auxprotect_main_data", Table.AUXPROTECT_MAIN.toString(),
              false));
    }
    ...
    {
      plugin.info(
          "Normalizing a:hurt and a:kill data. Creating temp index, this may take a while.");

      sql.execute(connection, "CREATE INDEX idx_auxprotect_spam_data ON " + Table.AUXPROTECT_SPAM
          + " (action_id,data)");

      plugin.info("Migrating IPs");
      while (true) {
        var toUpdate = sql.query(connection, "SELECT DISTINCT data FROM " + Table.AUXPROTECT_SPAM
            + " WHERE action_id=5 AND data IS NOT NULL LIMIT 10000", rs -> {
          Set<String> newIDs = new HashSet<>();
          while (rs.next()) {
            String data = rs.getString("data");
        ...
        sql.executeBatch(connection, "UPDATE " + Table.AUXPROTECT_SPAM
                + " SET target_id=?,data=null WHERE action_id=5 AND (data=? OR data=?)",
            toUpdateMap.entrySet(), e -> List.of(e.getValue(), e.getKey(), "IP: " + e.getKey()));
      }

      plugin.info("Removing temp index");
      sql.execute(connection,
          sql.getDropIndexStatement("idx_auxprotect_spam_data", Table.AUXPROTECT_SPAM.toString(),
              false));
    }
    ...
    plugin.info(
        "Completion state stored. You can unset \"MigrateDataNormalization\" in config.yml.");
  }
