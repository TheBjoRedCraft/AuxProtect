...
public class SQLUserManager {
  ...
  private final BidiMapCache<Integer, String> usernames = new BidiMapCache<>(300000L, 300000L,
      true);
  ...
  public void updateUsernameAndIP(UUID uuid, String name, String ip)
    ...
    sql.executeTransaction(connection -> {
      ...
      if (sql.query(connection, "SELECT 1 FROM " + Table.AUXPROTECT_LONGTERM + " WHERE target_id=?",
          ResultSet::next, ipID)) {
  ...
  public String getUUIDFromUID(int uid) throws SQLException, BusyException {
    ...
    return sql.getUidManager().getValue(uid).orElse(null);
  }
  ...
  public void setPendingInventory(int uid, byte[] blob) throws SQLException, BusyException {
    ...
    sql.executeTransaction(connection -> {
      if (blob == null) {
        sql.execute(connection, "DELETE FROM " + Table.AUXPROTECT_USERDATA_PENDINV + " WHERE uid=?",
            uid);
      } else {
        try {
