...
public enum Table {
  ...
  // Utility tables
  AUXPROTECT_INVDIFF(
      BLOB_ID), AUXPROTECT_UIDS, AUXPROTECT_WORLDS, AUXPROTECT_API_ACTIONS, AUXPROTECT_VERSION, AUXPROTECT_MIGRATION_TASKS, AUXPROTECT_INVBLOB, AUXPROTECT_LASTS, AUXPROTECT_INVDIFFBLOB, AUXPROTECT_USERDATA_PENDINV, AUXPROTECT_TRANSACTIONS_BLOB, AUXPROTECT_ENUM_IDS;

  public static final long MIN_PURGE_INTERVAL = 1000L * 60L * 60L * 24L * 14L;
  public final ConcurrentLinkedQueue<DbEntry> queue = new ConcurrentLinkedQueue<>();
  ...
  public String toString() {
    if (SQLManager.getInstance() == null) {
  ...
  public boolean exists(IAuxProtect plugin) {
    if (plugin.getPlatform().getLevel() == PlatformType.Level.PROXY) {
      return switch (this) {
        case AUXPROTECT_MAIN, AUXPROTECT_COMMANDS, AUXPROTECT_CHAT, AUXPROTECT_LONGTERM,
             AUXPROTECT_API,
             AUXPROTECT_UIDS, AUXPROTECT_API_ACTIONS, AUXPROTECT_VERSION,
             AUXPROTECT_MIGRATION_TASKS, AUXPROTECT_ENUM_IDS -> true;
        default -> false;
      };
    }
  ...
  public String getSQLCreateString(IAuxProtect plugin) {
    ...
    stmt += getColumns(plugin.getPlatform()).entrySet().stream()
  ...
  void validateID(String name, int id, int idPos) throws IllegalArgumentException {
    ...
    if (idPos > 0 && idPos != id + 1) {
      throw new IllegalArgumentException(
          "idPos is not id+1: id=" + id + ", idPos=" + idPos + " for action: " + name);
    }
  }

  public List<String> getIndexStatements(PlatformType platform) {
    return Arrays.stream(Index.values()).filter(i -> i.exists(this, platform))
        .map(i -> i.getCreate(this)).toList();
  }
