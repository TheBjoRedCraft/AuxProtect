...
public class DbEntryGroup extends DbEntry {
  ...
  protected DbEntryGroup(DbEntry root, Parameters params) {
    super(0, 0, EntryAction.GROUPING, false, root.getWorld(), root.getX(), root.getY(), root.getZ(),
        root.getPitch(), root.getYaw(), null, 0, "", root.sql);
      if (root instanceof DbEntryGroup) {
          throw new IllegalArgumentException("Tried to add a group to a group!");
      }
    this.range = Math.pow(params.getGroupRange(), 2);
    try {
      this.params = params.clone().group(0).clearRadius().addRadius((int) Math.ceil(range), false)
          .setLocation(root.getWorld(), root.getX(), root.getY(), root.getZ());
