...
public final class AuxProtectAPI {
  ...
  /**
   * Gets the {@link LookupManager} which can be used to conduct lookups similarly to the `/ap
   * lookup` command.
   */
   ...
   * {@link EntryAction#getAction(String)} to determine if an action already exists, or to recall it
   * later. Actions only need to be created once, after which they are automatically loaded on
   * startup. You cannot delete actions. Be careful what you create.
   *
   * @param plugin The name of the plugin creating the action. This is only used to identify the
   *               owner of an action for debug purposes.
   * @param key    The key which will be used to refer to this action in lookups.
   * @param ntext  The text which will be displayed for this action in looks for negative actions.
   * @param ptext  The text which will be displayed for this action in looks for positive actions
   *               (e.g. +session), or null for singular actions.
   * @return The newly created EntryAction.
   * @throws AlreadyExistsException   if the action you are attempting to create already exists or
   *                                  the name is taken.
  ...
  public static synchronized EntryAction createAction(@Nonnull String plugin, @Nonnull String key,
