...
public class TownyListener implements Listener {
  ...
  public TownyListener(AuxProtectPaper plugin) {
    this.plugin = plugin;
  }

  private TownyManager townyManager() {
    // TownyListener is only registered when Towny is present (see AuxProtectPaper.hook()),
  ...
  private void handleDeleted(String name, boolean nation) {
    ...
    plugin.add(
        new TownyEntry("#server", nation ? EntryAction.NATIONDELETE : EntryAction.TOWNDELETE, false,
            uuid, ""));
  }
  ...
  public void on(NewTownEvent e) {
    ...
    try {
      plugin.add(new TownyEntry(AuxProtectPaper.getLabel(e.getTown().getMayor().getPlayer()),
  ...
  public void on(RenameTownEvent e) {
    townyManager().updateName(e.getTown(), true);
    plugin.add(
        new TownyEntry(AuxProtectPaper.getLabel(e.getTown().getMayor()), EntryAction.TOWNRENAME,
            false,
  ...
  public void on(DeleteTownEvent e) {
    ...
    plugin.add(
        new TownyEntry("$" + e.getMayorUUID(), EntryAction.TOWNDELETE, false, toLoc(e.getMayor()),
            "$t" + e.getTownUUID(), ""));
  }
  ...
  public void on(TownAddResidentEvent e) {
    plugin.add(
        new TownyEntry("$" + e.getResident().getUUID().toString(), EntryAction.TOWNJOIN, true,
  ...
  public void on(TownRemoveResidentEvent e) {
    plugin.add(
        new TownyEntry("$" + e.getResident().getUUID().toString(), EntryAction.TOWNJOIN, false,
            toLoc(e.getResident()), TownyManager.getLabel(e.getTown()), ""));
  }
  ...
  public void on(TownClaimEvent e) {
    try {
      plugin.add(
          new TownyEntry("$" + e.getResident().getUUID().toString(), EntryAction.TOWNCLAIM, true,
              toLoc(e.getTownBlock().getWorldCoord()),
              TownyManager.getLabel(e.getTownBlock().getTown()), ""));
  ...
  public void on(TownMayorChangeEvent e) {
    plugin.add(
        new TownyEntry(AuxProtectPaper.getLabel(e.getNewMayor().getPlayer()), EntryAction.TOWNMAYOR,
            false,
            toLoc(e.getNewMayor()), TownyManager.getLabel(e.getTown()),
  ...
  private void handleBank(BankTransactionEvent e, Government g, EntryAction action) {
    ...
    if (player != null) {
      user = AuxProtectPaper.getLabel(player);
  ...
  public void on(NewNationEvent e) {
    ...
    plugin.add(new TownyEntry(AuxProtectPaper.getLabel(king), EntryAction.NATIONCREATE, false,
  ...
  public void on(RenameNationEvent e) {
    ...
    plugin.add(new TownyEntry(AuxProtectPaper.getLabel(mayor), EntryAction.NATIONRENAME, false,
  ...
  public void on(DeleteNationEvent e) {
    ...
    plugin.add(
        new TownyEntry(e.getLeader() == null ? "" : ("$" + e.getLeader().getUUID()),
            EntryAction.TOWNDELETE, false, "$t" + e.getNationUUID(), ""));
  }

  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  public void on(NationAddTownEvent e) {
    plugin.add(new TownyEntry(TownyManager.getLabel(e.getTown()), EntryAction.NATIONJOIN, true,
        TownyManager.getLabel(e.getNation()), ""));
  }

  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  public void on(NationRemoveTownEvent e) {
    plugin.add(new TownyEntry(TownyManager.getLabel(e.getTown()), EntryAction.NATIONJOIN, false,
        TownyManager.getLabel(e.getNation()), ""));
  }
