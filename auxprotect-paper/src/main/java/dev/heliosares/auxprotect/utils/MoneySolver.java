...
public class MoneySolver extends ChartRenderer {
  ...
  private MoneySolver(AuxProtectPaper plugin, Player player, List<DbEntry> results, int time,
      String user)
      throws IllegalArgumentException {
    super(plugin, user + "'" + (user.toLowerCase().endsWith("s") ? "" : "s") + " Money",
        Color.LIGHT_GRAY, 100);
    if (results.size() < 3) {
  ...
  public static void showMoney(IAuxProtect plugin, Player player, List<DbEntry> results, int time,
      String users) {
    if (!(plugin instanceof AuxProtectPaper)) {
      return;
    }
    AuxProtectPaper.getMorePaperLib().scheduling().entitySpecificScheduler(player).run(() -> {
      try {
        MoneySolver solver = new MoneySolver(AuxProtectPaper.getInstance(), player, results, time,
            users);
  ...
  public void drawXDivs(MapView view, MapCanvas canvas, Player player) {
    ...
    for (int x = 0; x < xDivs.size() + 1; x++) {
      setPixelColor(canvas, (int) Math.round(x * pixelsPerDiv + xShift) - 1, ySize + yShift + 2,
          Color.BLACK);
    }
    ...
    for (int i = 0; i < numberOfLabels; i++) {
      String str =
          xDivs.get((int) Math.min(Math.ceil(inc * i), xDivs.size() - 1)).getDayOfMonth() + "";
