...
public class PlayerListener implements Listener {
  ...
  public PlayerListener(AuxProtectPaper plugin) {
    this.plugin = plugin;
  ...
  public static void logMoney(AuxProtectPaper plugin, Player player, String reason) {
    if (plugin.getEconomy() == null) {
      return;
    }
    plugin.getAPPlayer(player).lastLoggedMoney = System.currentTimeMillis();
    try {
      plugin.add(new SpigotDbEntry(AuxProtectPaper.getLabel(player), EntryAction.MONEY, false,
          player.getLocation(),
  ...
  public void onPlayerItemDamageEvent(PlayerItemDamageEvent e) {
    ...
    if (item.getItemMeta() instanceof Damageable meta) {
      if (item.getType().getMaxDurability() - meta.getDamage() - e.getDamage() <= 0) {
        EntityListener.itemBreak(plugin, AuxProtectPaper.getLabel(e.getPlayer()), item,
  ...
  public void onPlayerInteractEntityEvent(PlayerInteractEntityEvent e) {
    ...
    if (item.getType() == Material.NAME_TAG) {
      ...
      if (meta != null && meta.hasDisplayName()) {
        plugin.add(
            new SpigotDbEntry(AuxProtectPaper.getLabel(e.getPlayer()), EntryAction.NAMETAG, true,
    ...
    if (e.getRightClicked() instanceof final ItemFrame itemFrame) {
      if (itemFrame.getItem().getType() == Material.AIR) {
        ...
        if (added.getType() != Material.AIR) {
          String data = "";
          DbEntry entry = new SingleItemEntry(AuxProtectPaper.getLabel(e.getPlayer()),
              EntryAction.ITEMFRAME, true,
  ...
  public void onPlayerInteractEvent(PlayerInteractEvent e) {
    plugin.getAPPlayer(e.getPlayer())
        .addActivity(e.getClickedBlock() == null ? Activity.INTERACT_AIR : Activity.INTERACT_BLOCK);
    ...
    if (e.getAction() == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock() != null) {
      if ((e.getItem() != null && buckets.contains(e.getItem().getType()))) {
        plugin.add(
            new SingleItemEntry(AuxProtectPaper.getLabel(e.getPlayer()), EntryAction.BUCKET, false,
  ...
  public void onEntityToggleGlideEvent(EntityToggleGlideEvent e) {
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(e.getEntity()), EntryAction.ELYTRA,
        e.isGliding(),
  ...
  public void onPlayerGameModeChangeEvent(PlayerGameModeChangeEvent e) {
    plugin.add(
        new SpigotDbEntry(AuxProtectPaper.getLabel(e.getPlayer()), EntryAction.GAMEMODE, false,
  ...
  public void onConsume(PlayerItemConsumeEvent e) {
    String sup = "";
    if (e.getItem().getType() == Material.POTION && e.getItem()
        .getItemMeta() instanceof PotionMeta pm) {
    ...
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(e.getPlayer()), EntryAction.CONSUME,
        false,
  ...
  public void onPlayerJoinEvent(PlayerJoinEvent e) {
    ...
    if (e.getPlayer().getAddress() != null) {
      ...
      String target = "";
      if (plugin.getAPConfig().isSessionLogIP()) {
        target = ip;
      }
      ...
      AuxProtectPaper.getMorePaperLib().scheduling().asyncScheduler().run(() -> {
        try {
          ...
          plugin.warning(
              "Database Busy: Unable to update username/ip for " + e.getPlayer().getName()
                  + ", this may cause issues with lookups but will resolve when they relog and the database is not busy.");
    ...
    AuxProtectPaper.getMorePaperLib().scheduling().asyncScheduler().runDelayed(() -> {
      ...
      } catch (SQLException | BusyException ex) {
        return;
      }
      ...
      message.append(Component.newline())
          .append(Component.text("         ")); // 9 Spaces

      if (e.getPlayer().getUniqueId().getMostSignificantBits() == 0) { // Bedrock Player
        message.append(
            Component.text(Language.L.COMMAND__INV__NOTIFY_PLAYER_CLAIM_ALT.translate()));
      } else {
        message.append(
            Component.text(Language.L.COMMAND__INV__NOTIFY_PLAYER_CLAIM_BUTTON.translate(),
                    NamedTextColor.GREEN)
                .clickEvent(ClickEvent.runCommand("/claiminv"))
                .hoverEvent(HoverEvent.showText(
                    Component.text(Language.L.COMMAND__CLAIMINV__CLAIM_BUTTON__HOVER.translate())
                )));
      }
  ...
  public void onPlayerTeleport(PlayerTeleportEvent e) {
    ...
    if (!plugin.getAPConfig().isInventoryOnWorldChange() || sameWorld) {
      return;
    }
  ...
  public void onPlayerKickEvent(PlayerKickEvent e) {
    plugin.add(new SpigotDbEntry(AuxProtectPaper.getLabel(e.getPlayer()), EntryAction.KICK, false,
        e.getPlayer().getLocation(), "", e.getReason()));
  }

  protected void logSession(Player player, boolean login, String target) {
    plugin.add(new SpigotDbEntry(AuxProtectPaper.getLabel(player), EntryAction.SESSION, login,
        player.getLocation(), target, ""));
  }

  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  public void onPlayerLeashEntityEvent(PlayerLeashEntityEvent e) {
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(e.getPlayer()), EntryAction.LEASH,
        true,
        e.getEntity().getLocation(), AuxProtectPaper.getLabel(e.getEntity()), "");
    plugin.add(entry);
  }

  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  public void onEntityPlaceEvent(EntityPlaceEvent e) {
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(e.getPlayer()), EntryAction.ENTITY,
        true,
        e.getEntity().getLocation(), AuxProtectPaper.getLabel(e.getEntity()), "");
    plugin.add(entry);
  }
  ...
  public void onVehicleDestroyEvent(VehicleDestroyEvent e) {
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(e.getAttacker()),
        EntryAction.ENTITY, false,
        e.getVehicle().getLocation(), AuxProtectPaper.getLabel(e.getVehicle()), "");
    plugin.add(entry);
  }
  ...
  public void onPlayerRespawnEvent(PlayerRespawnEvent e) {
    plugin.add(
        new SpigotDbEntry(AuxProtectPaper.getLabel(e.getPlayer()), EntryAction.RESPAWN, false,
  ...
  public void onCommand(PlayerCommandPreprocessEvent e) {
    ...
    plugin.add(
        new SpigotDbEntry(AuxProtectPaper.getLabel(e.getPlayer()), EntryAction.COMMAND, false,
  ...
  public void onChat(AsyncPlayerChatEvent e) {
    ...
    if (plugin.isDefaultChatLogging()) {
      plugin.add(
          new SpigotDbEntry(AuxProtectPaper.getLabel(e.getPlayer()), EntryAction.CHAT, false,
              e.getPlayer().getLocation(), "", e.getMessage().trim()));
    }
    if (plugin.getAPConfig().isDemoMode()) {
      e.getPlayer().sendMessage(NamedTextColor.RED + "Chat is disabled.");
      e.setCancelled(true);
    }
  }
