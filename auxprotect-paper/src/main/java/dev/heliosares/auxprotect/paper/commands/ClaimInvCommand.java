...
public class ClaimInvCommand implements CommandExecutor {
  ...
  public boolean onCommand(@Nonnull CommandSender sender, @Nullable Command command,
    ...
    plugin.runAsync(() -> {
      ...
      try {
        other = (args.length == 1) && APPermission.INV_RECOVER.hasPermission(
            new SpigotSenderAdapter(plugin, sender));
        if (other) {
          uid = plugin.getSqlManager().getUserManager().getUID(args[0], false);
          if (uid <= 0) {
          ...
          uid = plugin.getSqlManager().getUserManager().getUID("$" + player.getUniqueId(),
              false);
          target = player;
        } else {
        ...
        if (other) {
          ...
          sender.sendMessage(L.COMMAND__CLAIMINV__CANCELLED_OTHER.translate(target.getName(),
              Language.getOptionalS(target.getName())));
