...
public class MoneyCommand<S, P extends IAuxProtect, SA extends SenderAdapter<S, P>> extends
    Command<S, P, SA> {
  ...
  public void onCommand(SA sender, String label, String[] args) throws CommandException {
    ...
    sender.executeCommand(
        String.format(plugin.getCommandPrefix() + " lookup #money user:%s time:%s action:money",
            args[1], time));
  }
