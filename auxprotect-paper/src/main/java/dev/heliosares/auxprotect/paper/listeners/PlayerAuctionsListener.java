...
public class PlayerAuctionsListener implements Listener {
  ...
  public PlayerAuctionsListener(AuxProtectPaper plugin) {
    this.plugin = plugin;
  }

  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  public void onListItemEvent(PlayerAuctionSellEvent e) {
    DbEntry entry = new SingleItemEntry("$" + e.getSeller().getUniqueId(), EntryAction.AUCTIONLIST,
        false, e.getSeller().getLocation(),
        e.getPlayerAuction().getItem().getType().toString().toLowerCase(),
        plugin.formatMoney(e.getPlayerAuction().getPrice()), e.getPlayerAuction().getItem());
    plugin.add(entry);
  }

  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  public void onPurchaseItemEvent(PlayerAuctionBuyEvent e) {
    DbEntry entry = new SingleItemEntry("$" + e.getBuyer().getUniqueId(), EntryAction.AUCTIONBUY,
        false, e.getBuyer().getLocation(),
        e.getPlayerAuction().getItem().getType().toString().toLowerCase(),
        "From " + e.getPlayerAuction().getAuctionPlayer().getName() + " for " + plugin.formatMoney(
            e.getPrice()), e.getPlayerAuction().getItem());
    plugin.add(entry);
  }
}
