...
public class InventoryListener implements Listener {
  ...
  public InventoryListener(AuxProtectPaper plugin) {
    this.plugin = plugin;
  }
  ...
  private void log(HumanEntity player, Inventory inv, boolean state) {
    ...
    String label = AuxProtectPaper.getLabel(inv.getHolder());
    ...
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(player), EntryAction.INV, state,
        location, label, count + " items in inventory.");
    plugin.add(entry);
  }
  ...
  public void onEnchantItemEvent(EnchantItemEvent e) {
    ...
    plugin.add(
        new SingleItemEntry(AuxProtectPaper.getLabel(e.getEnchanter()), EntryAction.ENCHANT, false,
            e.getEnchanter().getLocation(), item.getType().toString(), "", item));
  }
  ...
  public void onInventoryClick(InventoryClickEvent e) {
    ...
    if (e.getClick() == ClickType.SHIFT_LEFT || e.getClick() == ClickType.SHIFT_RIGHT) {
      ...
      for (ItemStack item : e.getWhoClicked().getInventory().getStorageContents()) {
        if (item == null
            || item.isSimilar(e.getCurrentItem()) && item.getAmount() < item.getMaxStackSize()) {
          fits = true;
          break;
        }
      }
    ...
    if (type == InventoryType.ANVIL) {
      ...
      actuallyCrafted =
          (int) Math.ceil(actuallyCrafted / (double) recipeResultSize) * recipeResultSize;
    ...
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(e.getWhoClicked()), action, false,
        e.getWhoClicked().getLocation(), e.getCurrentItem().getType().toString().toLowerCase(),
        data);
    ...
    if (entryItems != null) {
      try {
        ...
        AuxProtectAPI.warning(
            "Failed to serialize item: " + e.getCurrentItem() + " at " + entry.getTime() + "e");
