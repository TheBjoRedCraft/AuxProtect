...
public class InventoryCommand<S, P extends IAuxProtect, SA extends SenderAdapter<S, P>> extends
  ...
  public void onCommand(SA sender, String label, String[] args) throws CommandException {
    ...
    if (!paramtime.startsWith("@")) {
      sender.executeCommand(String.format(
          plugin.getCommandPrefix() + " lookup user:%s action:" + EntryAction.INVENTORY
              + " time:%s", target, paramtime));
      return;
    }
  ...
  private AuxProtectPaper getPlugin() {
