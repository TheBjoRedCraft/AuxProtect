...
public class SaveInvCommand<S, P extends IAuxProtect, SA extends SenderAdapter<S, P>> extends
  ...
  public void onCommand(SA sender, String label, String[] args) throws CommandException {
    ...
    for (Player target : targets) {
      ...
      sender.sendLang(Language.L.COMMAND__SAVEINV__SUCCESS, target.getName(),
  ...
  public List<String> onTabComplete(SA sender, String label, String[] args) {
    if (args.length == 2 && plugin instanceof AuxProtectPaper spigot) {
      List<String> out = new ArrayList<>(
          spigot.getServer().getOnlinePlayers().stream().map(Player::getName).toList());
      if (APPermission.INV_SAVE_ALL.hasPermission(sender)) {
        out.add("*");
      }
      return out;
    }
