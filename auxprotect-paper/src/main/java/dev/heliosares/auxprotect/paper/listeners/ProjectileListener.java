...
public class ProjectileListener implements Listener {
  ...
  public ProjectileListener(AuxProtectPaper plugin) {
    this.plugin = plugin;
  ...
  private void logEntity(LivingEntity actor, Projectile entity, EntryAction action,
      boolean logData) {
    ...
    if (actor == null) {
      ...
      if (entity.getShooter() instanceof BlockProjectileSource shooter) {
        actorLabel = AuxProtectPaper.getLabel(shooter.getBlock());
      } else if (entity.getShooter() instanceof LivingEntity shooter) {
        actorLabel = AuxProtectPaper.getLabel(shooter);
      } else {
        return;
      }
    } else {
      actorLabel = AuxProtectPaper.getLabel(actor);
    }
    ItemStack item = null;
    ...
    DbEntry entry = new SingleItemEntry(actorLabel, action, false, entity.getLocation(),
        AuxProtectPaper.getLabel(entity), "", logData ? item : null);
    plugin.add(entry);
  }
}
