...
public class Telemetry {
  ...
  public static void init(AuxProtectPaper plugin, int pluginId) {
    Metrics metrics = new Metrics(plugin, pluginId);

    metrics.addCustomChart(
        new Metrics.SingleLineChart("entries", () -> plugin.getSqlManager().getCount()));
    ...
    metrics.addCustomChart(new Metrics.SimplePie("mysql",
        () -> plugin.getSqlManager().isMySQL() ? "MySQL" : "SQLite"));

    metrics.addCustomChart(
        new Metrics.SimplePie("db-version", () -> plugin.getSqlManager().getVersion() + ""));
    ...
    metrics.addCustomChart(new Metrics.SimplePie("updatechecker",
        () -> plugin.getAPConfig().shouldCheckForUpdates() ? "Enabled" : "Disabled"));
