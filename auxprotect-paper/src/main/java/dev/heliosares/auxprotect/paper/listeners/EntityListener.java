...
public class EntityListener implements Listener {
  ...
  public EntityListener(AuxProtectPaper plugin) {
    this.plugin = plugin;
  ...
  protected static void itemBreak(IAuxProtect plugin, String cause, ItemStack item,
      Location location) {
    DbEntry entry = new SingleItemEntry(cause, EntryAction.BREAKITEM, false, location,
        AuxProtectPaper.getLabel(item.getType()), "", item);
    plugin.add(entry);
  }
  ...
  public void onEntityDismountEvent(EntityDismountEvent e) {
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(e.getEntity()), EntryAction.MOUNT,
        false,
        e.getDismounted().getLocation(), AuxProtectPaper.getLabel(e.getDismounted()), "");
    plugin.add(entry);
  }
  ...
  public void onEntityMountEvent(EntityMountEvent e) {
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(e.getEntity()), EntryAction.MOUNT,
        true,
        e.getMount().getLocation(), AuxProtectPaper.getLabel(e.getMount()), "");
    plugin.add(entry);
  }
  ...
  public void onEntityExplodeEvent(EntityExplodeEvent e) {
    ...
    if (e.getEntity() instanceof TNTPrimed tnt && tnt.getSource() != null) {
      if (tnt.getSource() instanceof Projectile proj && proj.getShooter() != null) {
        cause = AuxProtectPaper.getLabel(proj.getShooter());
      } else {
        cause = AuxProtectPaper.getLabel(tnt.getSource());
      }
    } else if (e.getEntity() instanceof LivingEntity) {
      if (e.getEntity() instanceof Monster monster && monster.getTarget() != null) {
        cause = AuxProtectPaper.getLabel(monster.getTarget());
      } else {
        cause = AuxProtectPaper.getLabel(e.getEntity());
      }
    ...
    DbEntry entry = new SpigotDbEntry(cause, EntryAction.EXPLODE, true, e.getEntity().getLocation(),
        AuxProtectPaper.getLabel(e.getEntity()), "");
    plugin.add(entry);
  }
  ...
  public void entityDamageByEntityEvent(EntityDamageByEntityEvent e) {
    if (e.getEntity() instanceof final ItemFrame item) {
      DbEntry entry = new SingleItemEntry(AuxProtectPaper.getLabel(e.getDamager()),
          EntryAction.ITEMFRAME, false,
    ...
    String targetName = AuxProtectPaper.getLabel(e.getEntity());
    String sourceName = AuxProtectPaper.getLabel(source);
    ...
    DbEntry entry = new SpigotDbEntry(sourceName, action, false, e.getEntity().getLocation(),
        targetName, itemname);
    plugin.add(entry);
  }
  ...
  public void entityDamageEvent(EntityDamageEvent e) {
    ...
    DbEntry entry = new SpigotDbEntry("#env", reason, false, e.getEntity().getLocation(),
        targetName,
  ...
  public void onResurrect(EntityResurrectEvent e) {
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(e.getEntity()), EntryAction.TOTEM,
        false,
  ...
  public void onPickupEvent(EntityPickupItemEvent e) {
    if (e.getEntity() instanceof Player player) {
      ...
      if (isChartMap(e.getItem().getItemStack()) && !APPermission.LOOKUP_MONEY.hasPermission(
          new SpigotSenderAdapter(plugin, player))) {
        e.setCancelled(true);
        e.getItem().remove();
      }
    }
  ...
  private void drop(Entity entity, Location loc, ItemStack item, boolean drop) {
    ...
    if (InvSerialization.isCustom(item)) {
      entry = new SingleItemEntry(AuxProtectPaper.getLabel(entity),
          drop ? EntryAction.DROP : EntryAction.PICKUP, false,
          loc, item.getType().toString().toLowerCase(), "", item);
    } else {
      entry = new PickupEntry(AuxProtectPaper.getLabel(entity),
          drop ? EntryAction.DROP : EntryAction.PICKUP,
  ...
  public void onTame(EntityTameEvent e) {
    DbEntry entry = new SpigotDbEntry(AuxProtectPaper.getLabel(e.getOwner()), EntryAction.TAME,
        false,
