...
public class SpigotDbEntry extends DbEntry {
  ...
  /**
   * @param userLabel   The label of the user, provided by AuxProtect#getLabel(Object) and
   *                    AuxProtectBungee#getLabel(Object) as applicable. This may also be a generic
   *                    string such as "#env"
   * @param state       Specifies the state of EntryAction (i.e +mount vs -mount), if applicable,
   *                    otherwise false.
   * @param location    May be null
   * @param targetLabel The label of the target, see userLabel for details.
   * @param data        Extra data about your entry. This is stored as plain text so use sparingly.
   */
  public SpigotDbEntry(String userLabel, EntryAction action, boolean state,
      @Nullable Location location, String targetLabel,
      String data) throws NullPointerException {
    super(userLabel, action, state,
  ...
  protected SpigotDbEntry(long time, int uid, EntryAction action, boolean state, String world,
      int x, int y, int z,
