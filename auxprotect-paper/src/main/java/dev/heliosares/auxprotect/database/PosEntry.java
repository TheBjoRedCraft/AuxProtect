...
public class PosEntry extends SpigotDbEntry {
  ...
  public PosEntry(String userUuid, EntryAction action, boolean state, Location location,
      String target) {
  ...
  public PosEntry(long time, int uid, EntryAction action, boolean state, String world, int x, int y,
      int z, byte increment, int pitch, int yaw, String target, int target_id, String data) {
  ...
  public PosEntry(long time, int uid, Location location) {
    super(time, uid, EntryAction.POS, false, Objects.requireNonNull(location.getWorld()).getName(),
        ...
        (int) Math.round(location.getZ()), Math.round(location.getPitch()),
        Math.round(location.getYaw()), "", -1, "", SQLManager.getInstance());
  ...
  /**
   * Stores the fraction of the x/y/z values into a single byte. The structure is as follows 0b X X
   * X Y Y Z Z Z X and Z are stored in 8ths, Y is stored in 4ths.
   */
  ...
  /**
   * Retrieves the fractional values from the increment byte generated in
   * {@link #getFractionalByte(double, double, double)}
   *
  ...
  public Component coordinatesComponent(SenderAdapter<?, ?> sender) {
    ...
    builder.append(Component.newline())
        ...
        .append(Component.text(
            String.format("(x%d/y%d/z%d/%s)", getX(), getY(), getZ(), getWorld()),
            NamedTextColor.GRAY));
    ...
    builder.append(
        Component.text(String.format(" (p%d/y%d)", getPitch(), getYaw()), NamedTextColor.GRAY));
