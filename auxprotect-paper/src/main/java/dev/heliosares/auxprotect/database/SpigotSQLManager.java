...
public class SpigotSQLManager extends SQLManager {
  ...
  public SpigotSQLManager(AuxProtectPaper plugin, String host, String database, String prefix,
    ...
    getLookupManager().addLoader(new EntryLoader(
        ...
        data -> new PosEntry(data.time(), data.uid(), data.action(), data.state(), data.world(),
            data.x(), data.y(), data.z(), data.rs().getByte("increment"), data.pitch(), data.yaw(),
            data.target(), data.target_id(), data.data()
        )));
    ...
    getLookupManager().addLoader(new EntryLoader(
        ...
        data -> {
          ...
          return new SingleItemEntry(data.time(), data.uid(), data.action(), data.state(),
              data.world(), data.x(), data.y(), data.z(), data.pitch(), data.yaw(), data.target(),
              data.target_id(), data.data(), qty, damage);
        }
    ));
    ...
    getLookupManager().addLoader(new EntryLoader(
        ...
        data -> {
          ...
          return new TransactionEntry(data.time(), data.uid(), data.action(), data.state(),
              data.world(), data.x(), data.y(), data.z(), data.pitch(), data.yaw(),
              data.target_id(), data.data(), quantity, cost, balance, target_id2, this);
        }
    ));
  }
  ...
  public DbEntry convertToTransactionEntryForMigration(DbEntry entry, EntryAction action,
    ...
    return new TransactionEntry(entry.getTime(), entry.getUid(), action, entry.getState(),
        entry.getWorld(), entry.getX(), entry.getY(), entry.getZ(), 0, 0, entry.getTargetId(), "",
        quantity, cost, balance, target_id2, this);
  }
}
