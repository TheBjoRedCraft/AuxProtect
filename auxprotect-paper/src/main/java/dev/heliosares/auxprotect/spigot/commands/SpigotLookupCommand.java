...
public class SpigotLookupCommand extends
  ...
  public SpigotLookupCommand(AuxProtectPaper plugin) {
    super(plugin);
  }

  @Override
  protected void count(List<DbEntry> rs, CountResult countResult)
    ...
    for (DbEntry entry : rs) {
      if (entry instanceof TransactionEntry transactionEntry) {
        countResult.totalMoney +=
            transactionEntry.getCost() * (transactionEntry.getState() ? -1 : 1);
      }
    }
  }

  @Override
  protected void handleResults(Parameters params_, List<DbEntry> rs, SpigotSenderAdapter sender)
      throws SQLException, BusyException, LookupException {
    if (params_.hasFlag(Parameters.Flag.MONEY)) {
      ...
      if (sender.getSender() instanceof org.bukkit.entity.Player player) {
        MoneySolver.showMoney(plugin, player, rs,
            (int) Math.round((double) params_.getAfter() / (1000 * 3600)),
